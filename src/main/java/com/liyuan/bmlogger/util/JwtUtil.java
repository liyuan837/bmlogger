...
public class JwtUtil {
     ...
     * @return
     */
    public static JwtUser checkLogin(String authorization) throws bmloggerException {
        JwtUser jwtUser;

        Claims claims = getClaims(authorization);
        if (claims == null) {
            throw new bmloggerException("无效token");
        }

        String usercode = (String) claims.get("usercode");
        if(usercode == null || usercode.equals("")){
            throw new bmloggerException("会话中的用户编号为空");
        }

        Long expiration = (Long) claims.get("expiration");
        if(expiration < (new Date().getTime())){
            throw new bmloggerException("会话超时，请重新登陆");
        }
